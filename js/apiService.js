...
export async function searchVideos(query, selectedSources = []) {
    // Debug日志
    console.log('[apiService.searchVideos] called:', { query, selectedSources, apiSites: Object.keys(API_SITES) });

    if (!validateApiInput(query, 'query')) {
        return { code: 400, msg: '无效的搜索参数', list: [] };
    }
    // selectedSources 判空提示
    if (!selectedSources || !Array.isArray(selectedSources) || selectedSources.length === 0) {
        console.warn('[apiService.searchVideos] selectedSources为空！不会发送任何请求');
        return { code: 204, msg: '未选择任何数据源', list: [] };
    }
    ...
    for (const src of selectedSources) {
        if (src && typeof src === 'string' && src.startsWith('custom_')) {
            ...
            console.warn(`[apiService.searchVideos] selectedSource "`, src, `" 未命中 API_SITES`);
        }
    }

    console.log('[apiService.searchVideos] 构造出的 apis:', apis);
    ...
    if (!apis.length) {
        console.warn('[apiService.searchVideos] apis为空，所有selectedSources都未映射到API_SITES！');
        return { code: 204, msg: '无可用数据源', list: [] };
    }
    ...
    const promises = apis.map(async ({ code, apiUrl }) => {
        try {
            ...
            console.log('[apiService.searchVideos] 正在fetch:', url);
            ...
            console.error(`[apiService.searchVideos] fetch出错 for ${code}:`, e);
            return [];
        }
    });
    ...
    try {
        ...
        console.error('[apiService.searchVideos] Promise.all抓到异常:', e);
        allResults = [];
    }
    // 去重
    const seen = new Set();
    ...
    console.log('[apiService.searchVideos] 完成，allResults数量：', allResults.length);
