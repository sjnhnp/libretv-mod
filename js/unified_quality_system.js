...
async function comprehensiveQualityCheck(m3u8Url) {
    ...
    const detectionPromises = [
        tryParseM3u8Resolution(m3u8Url).then(r => ({ ...r, method: 'm3u8_resolution', priority: 1 })).catch(() => null),
        performVideoElementDetection(m3u8Url).then(r => ({ ...r, method: 'video_element', priority: 2 })).catch(() => null),
        simplePrecheckSource(m3u8Url).then(r => ({ ...r, method: 'simple_analysis', priority: 3 })).catch(() => null)
    ].filter(Boolean); // 过滤掉可能失败的promise

    const results = await Promise.all(detectionPromises);
    const validResults = results.filter(r => r && r.quality && !/未知|失败|超时/.test(r.quality));

    console.log('所有有效检测结果:', validResults);

    if (validResults.length === 0) {
        console.log('所有检测方法都失败，返回默认结果');
        return { quality: '1080p', loadSpeed: '检测失败', pingTime: -1 };
    }
    ...
    // 关键修复 #2: 采用更智能的合并逻辑
    const bestResult = { ...validResults[0] }; // 以最高优先级的结果为基础

    // 寻找最准确的网络速度测试结果
    const networkTestResult = validResults.find(r => r.method === 'simple_analysis' && r.loadSpeed && /\d+\s*(KB|MB)\/s/i.test(r.loadSpeed));

    // 如果最高优先级的结果没有提供真实的网络速度，则从网络测试结果中补充
    if (networkTestResult && (!bestResult.loadSpeed || !/\d+\s*(KB|MB)\/s/i.test(bestResult.loadSpeed))) {
        bestResult.loadSpeed = networkTestResult.loadSpeed;
        bestResult.pingTime = networkTestResult.pingTime;
    }

    // 如果最终都没有真实速度，则清理该字段
    if (bestResult.loadSpeed && !/\d+\s*(KB|MB)\/s/i.test(bestResult.loadSpeed)) {
        bestResult.loadSpeed = null;
    }
